...
public class ServerQuery_noPacketData implements Runnable {
    ...
    private static final String TAG = ServerQuery_noPacketData.class.getSimpleName();
     ...
     */
    public ServerQuery_noPacketData( Context c, Handler h ) {
        this.context = c;
        this.status = 0;
        this.handler = h;
    }
    ...
    private ServerInfo SI_parseResponseFromSRCDS(byte[] data) {        
        ...
        try {
            ...
            len = 0;
            while( data[pos++] != (byte)0x00 ) len++;
            byte[] mapBytes = new byte[len];
            byteBuffer.get(mapBytes, 0, mapBytes.length);
            byteBuffer.get();
            map = new String(mapBytes, "UTF-8");
    
            // Skip the next string (game server path)
            while( byteBuffer.get() != (byte)0x00 ) {}
    
            // Get the game description
            pos = byteBuffer.position();
            len = 0;
            while( data[pos++] != (byte)0x00 ) len++;
            byte[] gameBytes = new byte[len];
            byteBuffer.get(gameBytes, 0, gameBytes.length);
            byteBuffer.get();
            game = new String(gameBytes, "UTF-8");
    
            // Skip the next 2 bytes (Steam application ID)
            byteBuffer.position(byteBuffer.position()+2);
    
            // Get the current number of players and move to the next byte
            numPlayers = (int)byteBuffer.get();
    
            // Get the maximum number of players and move to the next byte
            maxPlayers = (int)byteBuffer.get(); 
            ...
            len = 0;
            while( data[pos++] != (byte)0x00 ) len++;
            byte[] versionBytes = new byte[len];
            byteBuffer.get(versionBytes, 0, versionBytes.length);
            byteBuffer.get();
            version = new String(versionBytes, "UTF-8");
    
            // If we're not at the end of the array then get the additional data
            if( byteBuffer.hasRemaining() ) {
                // This byte is the Extra Data Flag (EDF)
                int EDF = (int)byteBuffer.get();
    
                // Skip the port number if included (2 bytes)
                if( (EDF & 0x80) > 0 ) byteBuffer.position(byteBuffer.position()+2);
    
                // Skip the SteamID if included (8 bytes)
                if( (EDF & 0x10) > 0 ) byteBuffer.position(byteBuffer.position()+8);
    
                // Skip SourceTV information if included (2 bytes and a string)
                if( (EDF & 0x40) > 0 ) {
                    byteBuffer.position(byteBuffer.position()+2);
                    while( byteBuffer.get() != 0x00 ) {};
                }
                ...
                if( (EDF & 0x20) > 0 ) {
                    ...
                    len = 0;
                    while( data[pos++] != (byte)0x00 ) len++;
                    byte[] tagBytes = new byte[len];
                    byteBuffer.get(tagBytes, 0, tagBytes.length);
                    byteBuffer.get();
                    tags = new String(tagBytes, "UTF-8");
                }
