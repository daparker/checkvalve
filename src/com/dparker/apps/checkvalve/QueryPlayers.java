...
public class QueryPlayers extends Activity
{
	...
	public void queryPlayers() throws Exception
	{
		...
		try
		{
            ...
            for( int x = 0; x < bufferIn.length; x++ ) { bufferIn[x] = tempBuffer[x]; }
	        ...
	        {
	            /*
	             * If there are multiple packets, each packet will have 12 header bytes,
	             * but the "first" packet (packet 0) will have an additional 6 header
	             * bytes.  UDP packets can arrive in any order, so we need to check the
	             * sequence number of each packet to know how many header bytes to strip.
	             */
	        ...
	        for( int i = 0; i < numpackets; i++ )
	        {
                ...
                {                	
	                ...
	                while( bytes[byteNum] != 0x00 ) { name += (char)bytes[byteNum++]; }
	                byteNum++;
	                
	                kills = (bytes[byteNum]) | (bytes[byteNum+1]>>8) | (bytes[byteNum+2]>>16) | (bytes[byteNum+3]>>24);
	                byteNum += 4;
	        		
	                time = Float.intBitsToFloat(
	                			(int)(
	                					(long)(bytes[byteNum] & 0xff) |
	                					(long)((bytes[byteNum+1] & 0xff)<<8) |
	                					(long)((bytes[byteNum+2] & 0xff)<<16) |
	                					(long)((bytes[byteNum+3] & 0xff)<<24)
	                			     )
	                		);
		            ...
		            playerName.setLayoutParams(new LayoutParams(
		            ...
		            numKills.setLayoutParams(new LayoutParams(
		                    LayoutParams.WRAP_CONTENT,
		                    LayoutParams.WRAP_CONTENT));
		            ...
		            connected.setLayoutParams(new LayoutParams(
		                    LayoutParams.WRAP_CONTENT,
		                    LayoutParams.WRAP_CONTENT));
